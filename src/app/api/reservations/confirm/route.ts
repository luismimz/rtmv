...
export async function GET(request: Request) {
  try {
    ...
    console.error("Error mostrando la confirmación de reserva:", error);
    return htmlResponse(unexpectedErrorPage(), 500);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error("Error procesando la confirmación de reserva:", error);
    return htmlResponse(unexpectedErrorPage(), 500);
  }
}
...
function unexpectedErrorPage() {
  return renderActionPage({
    ...
    message:
      "No hemos podido procesar esta solicitud. Vuelve a intentarlo desde el correo o contacta directamente con el cliente.",
    tone: "error",
  });
}
